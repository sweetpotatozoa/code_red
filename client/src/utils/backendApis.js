...
6. 미리보기 json의 steps 배열은 반드시 첫음엔 type이 welcome인 step이, 마지막엔 type이 thank인 step이 있어야해. 만약 welcome이랑 thank를 쓰지 않더라도 존재해하되 isActive가 false여야해.
`,
    })
  }
  ...
  async startChatConversation(history, message) {
    ...
    const cleanedConversationPart = conversationPart
      .replace(/\*\*/g, '') // '**' 제거
      .replace(/\n{2,}/g, '<br />') // 연속적인 줄바꿈을 하나의 <br />로 변환
      .replace(/\n/g, '<br />') // 나머지 단일 줄바꿈을 <br />로 변환
